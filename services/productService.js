...
async function getProductById(productId) {
  ...
  // 若找不到，回傳 null
  const products = await fetchProducts();
  return products.find((items) => items.id === productId) || null ;
}
...
function displayProducts(products) {
  ...
  console.log(`    分類：${items.title}`);
  console.log(`    原價：${formatCurrency(items.original_price)}`);
  console.log(`    售價：${formatCurrency(items.price)} (${getDiscountRate(items)}) `);
  console.log(` ----------------------------------------`);
  });
}
